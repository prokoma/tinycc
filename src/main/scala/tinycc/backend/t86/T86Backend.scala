...
class T86Backend(program: IrProgram, machineRegCount: Int, machineFRegCount: Int) extends Logging {
  def result(): T86Listing = {
    ...
    profile("registerAllocation", T86RegisterAllocator(machineRegCount, machineFRegCount).transformProgram(t86Program))
