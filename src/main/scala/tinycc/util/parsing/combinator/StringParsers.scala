...
/**
  ...
  */
class CharInput(source: String, val start: Int, val end: Int) extends CharSequence with IndexedSeq[Char] with FiniteInput { outer =>
  ...
  override def drop(n: Int): CharInput = subSequence(n, length)
  ...
  override def apply(i: Int): Char = charAt(i)
}
